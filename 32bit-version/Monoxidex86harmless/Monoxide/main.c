...
)
{
	...
	Initialize( );
	if (MessageBoxW(NULL, L"Run GDI?", L"Monoxidex86harmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}

	if (MessageBoxW(NULL, L"Are you sure?", L"Monoxidex86harmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}

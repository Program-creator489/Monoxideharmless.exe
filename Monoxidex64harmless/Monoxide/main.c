...
)
{
	...
	Initialize( );
	if (MessageBoxW(NULL, L"Run GDI?", L"Monoxidex64harmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}

	if (MessageBoxW(NULL, L"Are you sure?", L"Monoxidex64harmless.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
